...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "java-learning-ear.ear");

        // Current directory is the root of the "java-learning-web" project. Go up one level
        // and enter the "java-learning-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../java-learning-ejb/target/java-learning-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "java-learning-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/java-learning-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "java-learning-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("org.learning.test");
